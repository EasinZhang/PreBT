...
from datasets.dataset import ImputationDataset, TransductionDataset, ClassiregressionDataset, DatasetforSOT,DatasetforSOC, DatasetforSOH, collate_unsuperv, \
    collate_superv,collate_supervforSOH, collate_supervforSOT, collate_supervforSOC
...
def pipeline_factory(config):
    ...
    if task == "imputation":
        return partial(ImputationDataset, mean_mask_length=config['mean_mask_length'],
                       ...
                       distribution=config['mask_distribution'], exclude_feats=config['exclude_feats'], forecast=config['forecast']), \
    ...
    if (task == "regression") and (dataset == "sotdataset"):
        return DatasetforSOT, collate_supervforSOT, SupervisedRunner
    if (task == "regression") and (dataset == "socdataset"):
        return DatasetforSOC, collate_supervforSOC, SupervisedRunner
    if (task == "regression") and (dataset == "sohdataset"):
        return DatasetforSOH, collate_supervforSOH, SupervisedRunner
    else:
...
def apply_revin(lengths, X, targets, target_masks, revin):
...
def cancel_revin(predictions, revin):
    predictions = revin(predictions, mode='denorm')
...
class UnsupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            lengths, X, targets, target_masks, padding_masks, IDs = batch
            ...
            #     X, targets = apply_revin(lengths, X, targets, target_masks, self.revin)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        if keep_all:
            per_batch = {'target_masks': [], 'targets': [], 'predictions': [], 'metrics': [], 'IDs': [],'padding_masks':[]}
        for i, batch in enumerate(self.dataloader):
            ...
            predictions = self.model(X, padding_masks)  # (batch_size, padded_length, feat_dim)
            # if self.revin is not None:
            #     predictions = cancel_revin(predictions, self.revin)
            # Cascade noise masks (batch_size, padded_length, feat_dim) and padding masks (batch_size, padded_length)
            target_masks = target_masks * padding_masks.unsqueeze(-1)
            loss = self.loss_module(predictions, targets,
                                    target_masks)  # (num_active,) individual loss (square error per element) for each active value in batch
            ...
            if keep_all:
                ...
                per_batch['padding_masks'].append(target_masks.cpu().detach().numpy())
...
class SupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            targets = targets.to(self.device, dtype=torch.float32).squeeze(-1)

            padding_masks = padding_masks.to(self.device)  # 0s: ignore
            # regression: (batch_size, num_labels); classification: (batch_size, num_classes) of logits

            predictions = self.model(X.to(self.device), padding_masks).squeeze(-1)


            # if len(predictions.shape) == 2:
            #     loss = self.loss_module(predictions, targets, padding_masks)  # (batch_size,) loss for each sample in the batch
            # else:
            #     loss = self.loss_module(predictions, targets)
            loss = self.loss_module(predictions, targets)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            per_batch['IDs'].append(IDs)
            per_batch['padding_masks'].append(padding_masks.cpu().detach().numpy())
