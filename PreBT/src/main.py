...
from datasets.data import data_factory, Normalizer, RevIN
from datasets.datasplit import split_dataset, split_dataset_order
from models.ts_transformer import model_factory
from models.loss import get_loss_module
from optimizers import get_optimizer
from utils.picture import plot_results
...
def main(config):
    ...
    if config['test_only'] == 'testset':  # Only evaluate and skip training
        ...
        test_evaluator = runner_class(model, test_loader, device, loss_module,
                                            print_interval=config['print_interval'], console=config['console'], revin=revin)
        ...
        #Picture the results (include denormal)
        plot_results(config, per_batch_test['predictions'], per_batch_test['targets'], per_batch_test['padding_masks'], normalizer)
    ...
    def adjust_learning_rate(optimizer, epoch, warmup_epochs, total_epochs, initial_lr, min_lr=1e-6):
        """Decay the learning rate with half-cycle cosine after warm up"""
    ...
    warmup_epochs = total_epochs * config['warmup_ration']  # Set warmup epochs
    min_lr = config['min_lr']

    for epoch in tqdm(range(start_epoch + 1, total_epochs + 1), desc='Training Epoch', leave=False):
        ...
        lr = adjust_learning_rate(optimizer, epoch, warmup_epochs, total_epochs, initial_lr, min_lr)
        logger.info('Learning rate for epoch {}: {:.6f}'.format(epoch, lr))
        # Add learning rate to TensorBoard
        tensorboard_writer.add_scalar('learning_rate', lr, epoch)


        # evaluate if first or last epoch or at specified interval
        if (epoch == total_epochs) or (epoch == start_epoch + 1) or (epoch % config['val_interval'] == 0):
